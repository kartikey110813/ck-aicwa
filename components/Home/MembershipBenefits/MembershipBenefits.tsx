...
const MembershipBenefits = () => {
  ...
  const y = useTransform(scrollYProgress, [0, 1], ["3vh", "-150vh"]);
    ...
    >
      ...
      <div className="sticky top-0 h-screen flex flex-col md:flex-row items-stretch overflow-hidden py-10 md:px-20">
        ...
        <div className="md:w-1/2 relative z-10 md:mt-20 md:mb-20 overflow-hidden">
          <motion.div style={{ y }} className="flex flex-col space-y-4">
            {benefits.map((benefit) => (
              <div
                key={benefit.id}
                onMouseEnter={() => setSelected(benefit)}
                className={`relative cursor-pointer rounded-[20px] shadow-lg px-6 py-3 bg-white items-center hover:scale-[1.02] transition-all border-l-8 ${
                  selected.id === benefit.id
                    ? "border-red-600"
                    : "border-transparent"
                }`}
              >
                <h3 className="text-[25px] mb-2">{benefit.title}</h3>
        ...
        <div className="md:w-[60vw] bg-gradient-to-b from-[#7a0e10] via-[#8b1417] to-[#3b0b0d] text-white p-8 relative -ml-10 md:-ml-24 z-0 border-t-[10px] border-[#690714]">
          ...
          <div className="flex flex-col h-full justify-between text-start md:pl-[15%]">
            <div>
              <h2 className="text-[24px] text-[#EDD8B5] md:text-[36px]">
                Benefits of Membership
              </h2>
              <p className="text-[18px] text-[#EDD8B5]">
            ...
            <div className="flex flex-row gap-10">
              {selected.stats.map((stat, index) => (
                <div
                  key={index}
                  className="px-5 py-5 rounded-[20px] bg-gradient-to-b from-[#b5211b] to-[#6e0c0b] text-center shadow-[inset_0_2px_6px_rgba(255,255,255,0.2),0_8px_15px_rgba(0,0,0,0.4)]"
                >
                  <h3 className="text-[#EDD8B5] text-[18px] mb-4">
                    {stat.label}
                  </h3>
